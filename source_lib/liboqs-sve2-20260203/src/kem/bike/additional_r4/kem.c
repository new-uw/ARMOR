...
_INLINE_ ret_t reencrypt(OUT m_t *m, IN const pad_e_t *e, IN const ct_t *l_ct)
{
  ...
  size_t i = 0;
  while (i < sizeof(*m)) {
      svbool_t pg = svwhilelt_b8(i, sizeof(*m));

      svuint8_t v_tmp = svld1(pg, &tmp.raw[i]);
      svuint8_t v_c1  = svld1(pg, &l_ct->c1.raw[i]);

      svuint8_t v_out = sveor_x(pg, v_tmp, v_c1);

      svst1(pg, &m->raw[i], v_out);
